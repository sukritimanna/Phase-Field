...
void stress_prod(fftw_complex sigma[3][3],fftw_complex epsilon[3][3],double kron_delta[3][3],double lambda,double mu,double mu_prime)
{
	int i,j,k,l;
	fftw_complex trace_epsilon;
	...
	for(i=0;i<=2;i++)
	{
		for(j=0;j<=2;j++)
		{
			...
			//printf("%lf+i%lf\t",creal(sigma[i][j]),cimag(sigma[i][j]));
		}			
		//printf("\n");
	}
}
